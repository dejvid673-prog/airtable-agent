...
if (-not (Test-Path $MappingFile)) {
    throw "Mapping file was not found: $MappingFile"
}

$Stamp = Get-Date -Format "yyyyMMdd-HHmmss"
if (-not $PlanFile) {
    $PlanFile = Join-Path $Root "runs\airtable-plan-$Stamp.json"
}
if (-not $ApprovalFile) {
    $ApprovalFile = Join-Path $Root "approvals\airtable-approval-$Stamp.json"
}
